...
class UpdateGameUserMetadataDto {
  ...
  bool operator ==(Object other) =>
      ...
      other is UpdateGameUserMetadataDto &&
  ...
  @override
  String toString() =>
      'UpdateGameUserMetadataDto[earlyAccess=$earlyAccess, ageRating=$ageRating, title=$title, sortTitle=$sortTitle, releaseDate=$releaseDate, description=$description, notes=$notes, averagePlaytime=$averagePlaytime, cover=$cover, background=$background, rating=$rating, launchParameters=$launchParameters, launchExecutable=$launchExecutable, installerExecutable=$installerExecutable, urlScreenshots=$urlScreenshots, urlTrailers=$urlTrailers, urlGameplays=$urlGameplays, urlWebsites=$urlWebsites, publishers=$publishers, developers=$developers, tags=$tags, genres=$genres]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "UpdateGameUserMetadataDto[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "UpdateGameUserMetadataDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return UpdateGameUserMetadataDto(
        ...
        publishers: json[r'publishers'] is List
            ? (json[r'publishers'] as List)
                .map((e) =>
                    e == null ? const <String>[] : (e as List).cast<String>())
                .toList()
            : const [],
        developers: json[r'developers'] is List
            ? (json[r'developers'] as List)
                .map((e) =>
                    e == null ? const <String>[] : (e as List).cast<String>())
                .toList()
            : const [],
  ...
  static List<UpdateGameUserMetadataDto> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<UpdateGameUserMetadataDto>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = UpdateGameUserMetadataDto.listFromJson(
          entry.value,
          growable: growable,
        );
      }
